...
def predict_post(
    ...
    predict_data = processed_predict(temporary_new_data)
...
def predict_get(
    ...
    print("[INFO] app.py: new_data: ", new_data)
    print("[INFO] app.py: new_data: ", new_data[new_data_count])
    predict_data = processed_predict(
        new_data, count=new_data_count
    )
    print("[INFO] app.py: predict_data: ", predict_data)
    prediction = classifier.predict(predict_data)
    print("[INFO] app.py: prediction: ", prediction)
    if prediction[0] == 0:
        prediction = 0  # pagará empréstimo
    else:
        prediction = 1  # NÃO pagará empréstimo
